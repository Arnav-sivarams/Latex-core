import { api, apiText } from '/static/api.js?v=flow-model-1';
import { state, savePreferences, cacheBuffer, clearBufferCache } from '/static/state.js?v=flow-model-1';
...
async function loadNavigation() {
  ...
  for (const team of teams) {
    ...
    const teamIsProjectContext = state.project?.collaboration?.team_id === team.id;
    const teamActive = teamIsProjectContext || state.activeTeam?.id === team.id;
    const row = node('div', `team-nav-row${teamActive ? ' active-context' : ''}`);
    const title = node('button', 'team-name button-reset', team.name); title.setAttribute('aria-current', teamIsProjectContext ? 'location' : 'false'); title.onclick = () => { state.activeTeam = team; loadNavigation().catch((error) => toast(readableError(error), true)); };
    const actions = node('button', 'team-actions-trigger', '⋯'); actions.type = 'button'; actions.title = `Actions for ${team.name}`; actions.setAttribute('aria-label', `Actions for ${team.name}`); actions.setAttribute('aria-expanded', 'false'); actions.onclick = (event) => { event.stopPropagation(); openTeamActions(team, projectsForTeam, actions); };
    row.append(title, actions);
    const target = team.group_type === 'mentor_group' ? mentorRoot : root; target.append(row);
    for (const project of projectsForTeam) { const button = node('button', `nav-item${state.project?.id === project.workspace_id ? ' active' : ''}`); button.setAttribute('aria-current', state.project?.id === project.workspace_id ? 'page' : 'false'); button.append(node('span', '', project.name)); button.onclick = () => { state.activeTeam = team; openProject(project.workspace_id); }; target.append(button); }
  }
...
function showFile() {
  ...
  $('#setMainButton').classList.toggle('hidden', !state.currentFile || !state.currentFile.endsWith('.tex') || state.project.main_file === state.currentFile || !canEdit(record));
...
async function openProject(id) {
  ...
  try {
    const project = await api(`/api/projects/${id}`); state.project = project; state.activeTeam = project.collaboration ? { id: project.collaboration.team_id } : null; state.currentFile = null; state.tabs.clear(); updateTop(); await loadNavigation(); renderTree();
...
function pathLooksValid(path) { return !!path && !path.startsWith('/') && !path.split('/').some((part) => !part || part === '.' || part === '..'); }
function closeFileDialogs() { ['#fileActionsDialog', '#renameDialog', '#deleteDialog'].forEach((selector) => { const dialog = $(selector); if (dialog.open) dialog.close(); }); }
async function openFileActions(path) { const record = state.project?.files.find((file) => file.path === path); if (!canEdit(record) || fileMutationActive) return; if (path === state.currentFile && !(await flushDirty())) return; fileActionPath = path; $('#fileActionPath').textContent = path; $('#setMainFileAction').classList.toggle('hidden', !path.endsWith('.tex') || state.project.main_file === path); $('#fileActionsDialog').showModal(); }
function openRenameDialog() { if (!fileActionPath) return; $('#fileActionsDialog').close(); $('#renameCurrentPath').value = fileActionPath; $('#renamePathInput').value = fileActionPath; $('#renameDialogError').textContent = ''; $('#renameDialog').showModal(); $('#renamePathInput').focus(); $('#renamePathInput').select(); }
async function renameFile(event) { event.preventDefault(); const from = fileActionPath; const to = $('#renamePathInput').value.trim(); if (!from || !state.project) return; if (!pathLooksValid(to)) { $('#renameDialogError').textContent = 'Enter a valid relative file path.'; return; } if (from === to) { $('#renameDialogError').textContent = 'Choose a different path.'; return; } if (fileMutationActive || !(await flushDirty())) return; const submit = $('#renameSubmit'); fileMutationActive = true; submit.disabled = true; try { const result = await api(`/api/projects/${state.project.id}/files/${encodeURI(from)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'If-Match': `"${state.project.version}"` }, body: JSON.stringify({ path: to }) }); state.project.version = result.version; const tab = state.tabs.get(from); if (tab) { state.tabs.delete(from); state.tabs.set(to, tab); } if (state.currentFile === from) state.currentFile = to; if (isTeam()) structuralPrivateProjects.add(state.project.id); closeFileDialogs(); fileActionPath = null; await refreshProject(); showFile(); toast(isTeam() ? 'File move staged privately' : 'File renamed'); } catch (error) { $('#renameDialogError').textContent = readableError(error); } finally { fileMutationActive = false; submit.disabled = false; } }
function openDeleteDialog() { if (!fileActionPath) return; $('#fileActionsDialog').close(); const unsaved = fileActionPath === state.currentFile && buffer()?.dirty; $('#deleteTitle').textContent = `Delete ${fileActionPath}?`; $('#deleteCopy').textContent = unsaved ? 'Unsaved edits will be discarded. This cannot be undone.' : 'This cannot be undone.'; $('#deleteDialogError').textContent = ''; $('#deleteDialog').showModal(); $('#deleteSubmit').focus(); }
async function deleteFile() { const path = fileActionPath; if (!path || !state.project || fileMutationActive) return; if (path === state.currentFile && !(await flushDirty())) return; const submit = $('#deleteSubmit'); fileMutationActive = true; submit.disabled = true; try { const result = await api(`/api/projects/${state.project.id}/files/${encodeURI(path)}`, { method: 'DELETE', headers: { 'If-Match': `"${state.project.version}"` } }); state.project.version = result.version; const wasCurrent = state.currentFile === path; state.tabs.delete(path); if (isTeam()) structuralPrivateProjects.add(state.project.id); closeFileDialogs(); fileActionPath = null; await refreshProject(); if (wasCurrent) { const next = state.project.main_file && state.project.files.some((file) => file.path === state.project.main_file) ? state.project.main_file : state.project.files.find((file) => isText(file.path))?.path; state.currentFile = null; if (next) await openFile(next); else clearEditor(); } else showFile(); toast(isTeam() ? 'File deletion staged privately' : 'File deleted'); } catch (error) { $('#deleteDialogError').textContent = readableError(error); } finally { fileMutationActive = false; submit.disabled = false; } }
async function setMainForPath(path) { if (!path || !path.endsWith('.tex') || !(await flushDirty())) return; try { const result = await api(`/api/projects/${state.project.id}/main`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ path, version: state.project.version }) }); state.project.version = result.version; if (isTeam()) structuralPrivateProjects.add(state.project.id); closeFileDialogs(); fileActionPath = null; await refreshProject(); showFile(); toast(isTeam() ? 'Main file staged privately' : 'Main file updated'); } catch (error) { toast(readableError(error), true); } }
async function setMain() { await setMainForPath(state.currentFile); }
...
async function openProjectDialog(team = null) {
  $('#projectDialogError').textContent = ''; $('#projectForm').reset();
  const teams = await api('/api/teams'); const select = $('#projectTeam'); select.replaceChildren();
  for (const candidate of teams) { const option = node('option', '', candidate.name); option.value = candidate.id; select.append(option); }
  const selectedTeam = team && teams.find((candidate) => candidate.id === team.id);
  $('#workspaceTeam').checked = !!selectedTeam; $('#workspacePersonal').checked = !selectedTeam;
  if (selectedTeam) select.value = selectedTeam.id;
  syncProjectWorkspace();
  state.templates = await api('/api/templates').catch(() => []); const options = $('#templateOptions'); options.replaceChildren(); const blank = templateChoice('', 'Blank project', 'A simple editable LaTeX document'); blank.querySelector('input').checked = true; options.append(blank); for (const template of state.templates) options.append(templateChoice(template.id, template.name, `${template.description || 'Create from template'}${template.main_file ? ` · Main: ${template.main_file}` : ''}`)); $('#projectDialog').showModal(); $('#projectNameInput').focus();
}
function selectedProjectTeam() { return $('#workspaceTeam').checked ? $('#projectTeam').value : null; }
function syncProjectWorkspace() { const team = !!selectedProjectTeam(); $('#teamWorkspacePicker').classList.toggle('hidden', !team); $('#importProjectButton').classList.toggle('hidden', team); }
...
async function createProject(event) { event.preventDefault(); const name = $('#projectNameInput').value.trim(); const template = $('#projectForm input[name=template]:checked')?.value; const teamId = selectedProjectTeam(); if (!name) return; if ($('#workspaceTeam').checked && !teamId) { $('#projectDialogError').textContent = 'Choose a team for this project.'; return; } try { let project; if (teamId) { const endpoint = template ? `/api/teams/${teamId}/templates/${template}/projects` : `/api/teams/${teamId}/projects`; project = await api(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name }) }); $('#projectDialog').close(); await openProject(project.workspace_id); } else { const endpoint = template ? `/api/templates/${template}/projects` : '/api/projects'; project = await api(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name }) }); $('#projectDialog').close(); await openProject(project.id); } toast('Project created'); } catch (error) { $('#projectDialogError').textContent = readableError(error); } }
async function importProject() { if (selectedProjectTeam()) { $('#projectDialogError').textContent = 'Team ZIP import is not available in this release.'; return; } const picker = $('#projectZip'); picker.click(); picker.onchange = async () => { const file = picker.files[0]; const name = $('#projectNameInput').value.trim(); if (!file || !name) { $('#projectDialogError').textContent = 'Enter a project name, then choose a ZIP.'; return; } try { const project = await api(`/api/projects/import?name=${encodeURIComponent(name)}`, { method: 'POST', headers: { 'Content-Type': 'application/zip' }, body: file }); $('#projectDialog').close(); await openProject(project.id); toast('Project imported'); } catch (error) { $('#projectDialogError').textContent = readableError(error); } }; }

function closeTeamActions() { const popover = $('#teamActionsPopover'); if (!activeTeamActions) return; activeTeamActions.trigger?.setAttribute('aria-expanded', 'false'); popover.classList.add('hidden'); activeTeamActions = null; }
function clampTeamActionsPopover() { if (!activeTeamActions) return; const popover = $('#teamActionsPopover'); const preferredTop = activeTeamActions.trigger.getBoundingClientRect().bottom + 6; popover.style.top = `${Math.max(8, Math.min(preferredTop, innerHeight - popover.offsetHeight - 8))}px`; }
async function openTeamActions(team, projects, trigger) {
  closeTeamActions(); activeTeamActions = { team, projects, trigger, projectId: projects.find((project) => project.workspace_id === state.project?.id)?.id || projects[0]?.id || null };
  $('#teamActionsTitle').textContent = `Team ${team.name}`; const select = $('#teamActionsProject'); select.replaceChildren();
  for (const project of projects) { const option = node('option', '', project.name); option.value = project.id; select.append(option); }
  select.value = activeTeamActions.projectId || ''; select.disabled = !projects.length; $('#projectAccessPicker').classList.toggle('hidden', !projects.length);
  trigger.setAttribute('aria-expanded', 'true'); const popover = $('#teamActionsPopover'); const rect = trigger.getBoundingClientRect(); popover.style.top = `${rect.bottom + 6}px`; popover.style.left = `${Math.max(8, Math.min(innerWidth - 430, rect.left - 340))}px`; popover.classList.remove('hidden'); await renderTeamActions(); clampTeamActionsPopover(); $('#teamActionsPopover button, #teamActionsPopover input, #teamActionsPopover select')?.focus();
}
async function renderTeamActions() {
  if (!activeTeamActions) return; const list = $('#teamMembersList'); list.replaceChildren(); $('#teamMemberError').textContent = '';
  const { team, projectId } = activeTeamActions; if (!projectId) { list.append(node('p', 'muted', 'Create a project to manage project roles.')); $('#teamMemberForm').classList.add('hidden'); clampTeamActionsPopover(); return; }
  try {
    const [teamMembers, projectMembers] = await Promise.all([api(`/api/teams/${team.id}/members`), api(`/api/team-projects/${projectId}/members`)]);
    activeTeamActions.teamMembers = teamMembers; activeTeamActions.projectMembers = projectMembers; const manager = projectMembers.some((member) => member.user_id === state.user.id && member.project_manager); $('#teamMemberForm').classList.toggle('hidden', !manager);
    const projectByUser = new Map(projectMembers.map((member) => [member.user_id, member]));
    for (const member of teamMembers) {
      const roles = projectByUser.get(member.user_id) || { writer: false, mentor: false, project_manager: false }; const row = node('div', 'member-row'); const identity = node('span', 'member-email', member.email); const roleControls = node('span', 'member-role-controls');
      for (const [key, label] of [['writer', 'Writer'], ['mentor', 'Mentor'], ['project_manager', 'Project Manager']]) { const chip = node('label', `role-chip${roles[key] ? ' selected' : ''}`); const input = document.createElement('input'); input.type = 'checkbox'; input.checked = roles[key]; input.disabled = !manager; input.setAttribute('aria-label', `${label} for ${member.email}`); input.onchange = () => updateMemberRoles(member, { ...roles, [key]: input.checked }); chip.append(input, node('span', '', label)); roleControls.append(chip); }
      const remove = node('button', 'member-remove', 'Remove'); remove.type = 'button'; remove.disabled = !manager || !projectByUser.has(member.user_id); remove.onclick = () => removeProjectAccess(member); row.append(identity, roleControls, remove); list.append(row);
    }
  } catch (error) { list.append(node('p', 'inline-error', readableError(error))); $('#teamMemberForm').classList.add('hidden'); }
  clampTeamActionsPopover();
}
async function updateMemberRoles(member, roles) { if (!activeTeamActions) return; try { if (!roles.writer && !roles.mentor && !roles.project_manager) { await removeProjectAccess(member); return; } await api(`/api/team-projects/${activeTeamActions.projectId}/members`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: member.email, ...roles }) }); toast('Project roles updated'); await renderTeamActions(); } catch (error) { const message = readableError(error); await renderTeamActions(); $('#teamMemberError').textContent = message; } }
async function removeProjectAccess(member) { if (!activeTeamActions) return; try { await api(`/api/team-projects/${activeTeamActions.projectId}/members/${member.user_id}`, { method: 'DELETE' }); toast('Project access removed'); await renderTeamActions(); } catch (error) { const message = readableError(error); await renderTeamActions(); $('#teamMemberError').textContent = message; } }
async function addTeamMember(event) { event.preventDefault(); if (!activeTeamActions) return; const email = $('#teamMemberEmail').value.trim(); const roles = { writer: $('#teamRoleWriter').checked, mentor: $('#teamRoleMentor').checked, project_manager: $('#teamRoleManager').checked }; if (!roles.writer && !roles.mentor && !roles.project_manager) { $('#teamMemberError').textContent = 'Select at least one project role.'; return; } try { const exists = activeTeamActions.teamMembers.some((member) => member.email.toLowerCase() === email.toLowerCase()); if (!exists) await api(`/api/teams/${activeTeamActions.team.id}/members`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, group_manager: false }) }); await api(`/api/team-projects/${activeTeamActions.projectId}/members`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, ...roles }) }); $('#teamMemberForm').reset(); toast('Member added'); await renderTeamActions(); } catch (error) { $('#teamMemberError').textContent = readableError(error); } }
...
function adminShellControls(hidden) { ['#publishButton', '#compileButton', '#saveButton', '#setMainButton'].forEach((selector) => $(selector).classList.toggle('hidden', hidden)); }
function showAdmin() {
  adminShellControls(true); $('#workspaceLink').classList.remove('hidden'); $('#projectTitle').textContent = 'Admin'; $('#saveState').classList.add('hidden'); const workspace = $('#workspace'); workspace.classList.add('admin-workspace'); workspace.replaceChildren(); const panel = node('main', 'admin-shell'); const nav = node('nav', 'admin-nav'); const content = node('section', 'admin-content'); const heading = node('div', 'admin-heading'); heading.append(node('div', 'wordmark', 'LaTeX Core Admin'), node('h1', '', 'Control plane'), node('p', 'muted', 'Global administration is separate from membership-scoped projects.'));
...
function bindResizer(selector, field, min, max) { const bar = $(selector); let start = 0; let value = 0; const move = (event) => { const delta = field === 'bottom' ? start - event.clientY : event.clientX - start; state.preferences[field] = Math.max(min, Math.min(max, value + delta)); applyLayout(); }; const end = () => { document.removeEventListener('pointermove', move); document.removeEventListener('pointerup', end); bar.classList.remove('dragging'); savePreferences(); }; bar.addEventListener('pointerdown', (event) => { start = field === 'bottom' ? event.clientY : event.clientX; value = state.preferences[field]; bar.classList.add('dragging'); document.addEventListener('pointermove', move); document.addEventListener('pointerup', end); }); }
  function handleAction(action) { const all = { 'toggle-sidebar': () => { state.preferences.sidebar = !state.preferences.sidebar; savePreferences(); applyLayout(); }, 'new-project': () => openProjectDialog(), 'new-team-project': () => { const team = activeTeamActions?.team; closeTeamActions(); if (team) openProjectDialog(team); }, 'close-team-actions': closeTeamActions, 'new-team': openTeamDialog, 'new-file': () => { if (state.project) { $('#fileDialogError').textContent = ''; $('#fileForm').reset(); $('#fileDialog').showModal(); $('#filePathInput').focus(); } }, save, compile, publish, help: openHelp, theme: cycleTheme, 'set-main': setMain, 'rename-move': openRenameDialog, 'set-main-file': () => setMainForPath(fileActionPath), 'delete-file': openDeleteDialog, 'confirm-delete': deleteFile, 'reload-pdf': () => { const frame = $('#pdfFrame'); if (frame.src) frame.src = frame.src.replace(/([?&]v=)\d+/, `$1${Date.now()}`); }, 'toggle-pdf': () => { state.preferences.pdf = true; savePreferences(); applyLayout(); }, 'toggle-bottom': toggleBottom, 'close-dialog': () => document.querySelectorAll('dialog[open]').forEach((dialog) => dialog.close()), 'import-project': importProject }; all[action]?.(); }
function bindEvents() {
  ...
  $('#fileForm').addEventListener('submit', (event) => { event.preventDefault(); createFile(); }); $('#renameForm').addEventListener('submit', renameFile); $('#projectForm').addEventListener('submit', createProject); $('#teamForm').addEventListener('submit', createTeam); $('#teamMemberForm').addEventListener('submit', addTeamMember); $('#workspacePersonal').addEventListener('change', syncProjectWorkspace); $('#workspaceTeam').addEventListener('change', syncProjectWorkspace); $('#teamActionsProject').addEventListener('change', () => { if (activeTeamActions) { activeTeamActions.projectId = $('#teamActionsProject').value; renderTeamActions(); } }); $('#commandSearch').addEventListener('input', () => { state.commandIndex = 0; renderCommands(); });
  document.addEventListener('click', (event) => { if (activeTeamActions && !$('#teamActionsPopover').contains(event.target) && !activeTeamActions.trigger.contains(event.target)) closeTeamActions(); });
  document.addEventListener('keydown', (event) => { if (event.key === 'Escape') { closeTeamActions(); document.querySelectorAll('dialog[open]').forEach((dialog) => dialog.close()); } if (!(event.ctrlKey || event.metaKey)) return; const key = event.key.toLowerCase(); if (key === 's') { event.preventDefault(); save(); } if (key === 'enter') { event.preventDefault(); compile(); } if (key === 'p') { event.preventDefault(); openQuickOpen(); } if (key === 'k') { event.preventDefault(); openPalette(); } });
...
async function bootAuthenticated(user) {
  ...
  if (user.account_type === 'admin' && !['/admin', '/workspace'].includes(location.pathname)) { location.assign('/admin'); return; }
