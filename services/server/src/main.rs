...
use persistence::{
    ...
    ProjectAccess, ProjectMetadataInput, ProjectRoles, PublishResult, QueueLimits, TeamFileRecord,
...
struct V2TemplateEditInput {
    ...
    description: Option<String>,
    main_file: String,
    #[serde(default = "default_front_matter_arrangement")]
    front_matter_arrangement: String,
}
fn default_front_matter_arrangement() -> String {
    String::new()
}
#[derive(Deserialize)]
struct SetMain {
...
fn router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            "/api/v2/papers/{paper_id}/project-metadata",
            get(v2_project_metadata).put(v2_save_project_metadata),
        )
        .route(
...
fn project_definition_bytes(
    automatic: &BTreeMap<String, serde_json::Value>,
    input: &ProjectMetadataInput,
) -> Bytes {
    let mut values = automatic.clone();
    values.insert(
        "project.executive_summary".into(),
        input
            .executive_summary
            .clone()
    ...
    values.insert(
        "project.source_code_snippets".into(),
        serde_json::json!(
            input
    ...
    let single_source_bindings = front_matter::legacy::single_source_bindings(&values).ok();
    for (key, value) in values {
        ...
        let command = key
            ...
            .collect::<String>();
        let escaped = value.as_array().map_or_else(
            || front_matter::escape_latex_text(value.as_str().unwrap_or_default()),
            |items| {
                items
                    .iter()
                    .filter_map(serde_json::Value::as_str)
                    .map(front_matter::escape_latex_text)
                    .collect::<Vec<_>>()
                    .join("\\\\\n")
            },
        );
        let _ = writeln!(output, "% canonical: {key}; origin: resolved");
        let _ = writeln!(
            output,
            "\\expandafter\\def\\csname LatexCore{command}\\endcsname{{{escaped}}}"
        );
    }
    if let Some(bindings) = single_source_bindings {
...
#[allow(clippy::too_many_lines)]
fn validate_project_metadata(input: &ProjectMetadataInput) -> Result<(), Response> {
    if input
        .executive_summary
        .as_ref()
        .is_some_and(|value| value.len() > 20_000)
    {
        return Err(error(
            StatusCode::BAD_REQUEST,
            "Executive summary exceeds 20,000 bytes.",
        ));
    }
    if input
        .project_type
        .as_deref()
        .is_some_and(|value| !matches!(value, "capstone" | "project-1"))
    {
        return Err(error(
            StatusCode::BAD_REQUEST,
            "Project type must be capstone or project-1.",
        ));
    }
    if input.datasets.len() > 50
        || input.source_code_snippets.len() > 50
        || input.publications.len() > 100
        || input.department_display_names.len() > 50
        || input.school_display_names.len() > 50
    {
        return Err(error(
            StatusCode::BAD_REQUEST,
            "Project metadata collection limit exceeded.",
        ));
    }
    if input
        .department_display_names
        .iter()
        .chain(&input.school_display_names)
        .any(|item| {
            item.id.trim().is_empty()
                || item.id.len() > 200
                || item.display_name.trim().is_empty()
                || item.display_name.chars().count() > 300
        })
    {
        return Err(error(
            StatusCode::BAD_REQUEST,
            "Department and School display names require an applicable identity and a name of at most 300 characters.",
        ));
    }
    if input.datasets.iter().any(|item| {
        item.name.trim().is_empty()
            || item.name.len() > 500
            || item.url.as_ref().is_some_and(|value| value.len() > 2_048)
            || item
                .description
                .as_ref()
        ...
        return Err(error(
            StatusCode::BAD_REQUEST,
            "Dataset names and optional details must be within documented limits.",
        ));
    }
    if input.source_code_snippets.iter().any(|item| {
        item.label.trim().is_empty()
            ...
            || item.code.len() > 50_000
    }) {
        return Err(error(
            StatusCode::BAD_REQUEST,
            "Source snippets require a label and are limited to 50,000 bytes each.",
        ));
    }
    let mut keys = BTreeSet::new();
    for publication in &input.publications {
        let valid_key = !publication.citation_key.is_empty()
            && publication.citation_key.len() <= 100
            && publication.citation_key.chars().all(|character| {
                character.is_ascii_alphanumeric() || matches!(character, ':' | '_' | '.' | '-')
            });
        if !valid_key || !keys.insert(publication.citation_key.as_str()) {
            return Err(error(
                StatusCode::BAD_REQUEST,
                "Publication citation keys must be unique and contain only letters, numbers, colon, underscore, dot, or hyphen.",
            ));
        }
        if !valid_project_publication(publication) {
            return Err(error(
                StatusCode::BAD_REQUEST,
                "Publication fields are invalid or exceed documented limits.",
            ));
        }
    }
    if serde_json::to_vec(input).map_or(true, |value| value.len() > 512 * 1024) {
        return Err(error(
            StatusCode::PAYLOAD_TOO_LARGE,
            "Project metadata exceeds 512 KiB.",
        ));
    }
    Ok(())
}

fn valid_project_publication(publication: &persistence::ProjectPublication) -> bool {
    matches!(
        publication.status.as_str(),
        "communicated" | "accepted" | "published"
    ) && (1000..=3000).contains(&publication.year)
        && !publication.authors.trim().is_empty()
        && !publication.title.trim().is_empty()
        && !publication.venue.trim().is_empty()
        && publication.authors.len() <= 2_000
        && publication.title.len() <= 2_000
        && publication.venue.len() <= 1_000
        && publication
            .doi
            .as_ref()
...
async fn v2_project_metadata(
    ...
    if let Err(error_value) = state
        ...
        .await
    {
        return front_matter_repository_error(error_value);
    }
    match state.front_matter.project_metadata(paper_id).await {
        Ok(value) => Json(value).into_response(),
...
async fn v2_save_project_metadata(
    ...
    if let Err(response) = validate_project_metadata(&input) {
        return response;
    }
        ...
        .await
    {
        ...
        Ok(_) => {
            return error(
                StatusCode::FORBIDDEN,
                "Only the Team Leader can save project metadata.",
            );
        }
    ...
    let automatic = match automatic_result {
        Ok(value) => value,
        Err(error_value) => return front_matter_repository_error(error_value),
    };
    let stored_definition = match state
        .blobs
        .put(project_definition_bytes(&automatic, &input))
        .await
    {
        Ok(value) => value,
        Err(_) => {
            return error(
                StatusCode::INTERNAL_SERVER_ERROR,
                "project metadata materialization failed",
            );
        }
    };
    let definition = ManagedFrontMatterFile {
        path: ".latex-core/frontmatter/Front-Matter.tex".into(),
        blob_hash: stored_definition.hash(),
        size_bytes: stored_definition.size_bytes(),
    };
    let mut version = match state
        .front_matter
        .save_project_metadata(principal.user_id(), paper_id, &input, &definition)
        .await
    {
        Ok(value) => value,
        Err(error_value) => return front_matter_repository_error(error_value),
    ...
    if let Some(pack_id) = detail
        .get("pack_id")
        .and_then(serde_json::Value::as_str)
        .and_then(|value| uuid::Uuid::parse_str(value).ok())
    {
        let (overrides, sections) = detail_choices(&detail);
        if let Ok(rendered_version) = render_team_front_matter(
            &state,
            principal.user_id(),
            paper_id,
            pack_id,
        ...
        .await
        {
            ...
            const WARNING: &str = "Project metadata was saved, but the managed Front Matter could not be refreshed. Reopen Document details to retry.";
...
async fn render_team_front_matter(
    ...
    let project = state
        ...
        .await
        .map_err(front_matter_repository_error)?;
    let mut project_input: ProjectMetadataInput = serde_json::from_value(
        project
            .get("values")
            .cloned()
    ...
    .unwrap_or_default();
    project_input.setup_complete = project
        .get("setup_complete")
        .and_then(serde_json::Value::as_bool)
        .unwrap_or(false);
    let project_definitions = project_definition_bytes(&automatic, &project_input);
    let mut files = Vec::with_capacity(rendered.files.len());
    for mut file in rendered.files {
        if file.path.as_str() == ".latex-core/frontmatter/Front-Matter.tex" {
            let mut combined = file.bytes.to_vec();
            combined.extend_from_slice(&project_definitions);
            file.bytes = Bytes::from(combined);
        }
...
async fn admin_v2_create_paper_team(
    ...
    {
        Ok((team, files)) => {
            let main_file = files.iter().find(|file| file.path == main_path).cloned();
            ...
            (
                StatusCode::CREATED,
                Json(serde_json::json!({
                    "team":team,"files":files,"main_file":main_file,
...
async fn capture_team_metadata(
    state: &AppState,
    paper_id: uuid::Uuid,
    ...
    let front_matter_state = state
        .front_matter
        .version_state(paper_id)
        .await
        .map_err(front_matter_repository_error)?;
    let resolved_front_matter = state
        .front_matter
        .archive_metadata(paper_id)
        .await
        .map_err(front_matter_repository_error)?;
    ...
    if let Some(object) = manifest.as_object_mut() {
        ...
        object.insert("project_metadata".to_owned(), project_metadata);
    }
    Ok(())
}
...
async fn capture_exact_v2_state(
    ...
    if paper.kind == persistence::PaperKind::Team {
        capture_team_metadata(state, paper.id, &mut manifest).await?;
    }
...
fn front_matter_repository_error(error_value: FrontMatterRepositoryError) -> Response {
    match error_value {
        ...
        FrontMatterRepositoryError::InvalidProjectMetadata(message) => {
            error(StatusCode::BAD_REQUEST, message)
        }
...
async fn admin_templates(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    for template in templates {
        ...
        response.push(serde_json::json!({
            ...
            "front_matter_arrangement":template.front_matter_arrangement,
...
async fn admin_v2_template_import(
    ...
    let single_source_compatible = imported
        .files
        .iter()
        .find(|file| file.path == main)
        .is_some_and(|file| front_matter::single_source_compatible(&file.bytes));
    ...
    if !matches!(
        ...
        return error(StatusCode::BAD_REQUEST, "invalid Front Matter arrangement");
    }
    if arrangement == "SEPARATE_FILES" && !front_matter_compatible {
        return error(
            StatusCode::UNPROCESSABLE_ENTITY,
        ...
        .await
    {
        Ok(()) => {
            if state
                .repo
                .set_template_front_matter_arrangement(id, arrangement)
                .await
                .is_err()
            {
                return error(
                    StatusCode::INTERNAL_SERVER_ERROR,
                    "template arrangement update failed",
                );
            }
...
async fn template_import_fields(
        ...
        .map_err(|_| error(StatusCode::BAD_REQUEST, "invalid template upload"))?
    {
        match field.name() {
            ...
            Some("arrangement") => {
                upload.arrangement = Some(field.text().await.map_err(|_| {
                    error(StatusCode::BAD_REQUEST, "invalid Front Matter arrangement")
                })?);
            }
...
async fn admin_v2_template_edit(
    ...
    let arrangement = if input.front_matter_arrangement.is_empty() {
        ...
        input.front_matter_arrangement.as_str()
    };
    if !matches!(
        arrangement,
        "REPORT_CONTENT_ONLY" | "SEPARATE_FILES" | "SINGLE_SOURCE"
    ...
    if arrangement == "SEPARATE_FILES" && !compatible {
        return error(
            StatusCode::UNPROCESSABLE_ENTITY,
            "Separate-file Front Matter requires the verified LATEX_CORE_FRONT_MATTER input marker in Main.",
        );
...
mod database_tests {
    ...
    async fn realtime_collaboration_converges_persists_recovers_and_enforces_access() {
        ...
        // This test owns its template fixture. Relying on another database test
        // to install a global fallback made isolated runs order-dependent.
        let template_id = uuid::Uuid::new_v4();
        let template_main = state
            .blobs
            .put(Bytes::from_static(
                b"\\documentclass{article}\n\\begin{document}\nRealtime fixture\n\\end{document}\n",
            ))
        ...
        state
            ...
            .create_template(
                template_id,
                &format!("Realtime fixture {template_id}"),
                None,
                Some("main.tex"),
                &[AppTemplateFileRecord {
                    path: "main.tex".into(),
                    blob_hash: template_main.hash(),
                    size_bytes: template_main.size_bytes(),
                }],
            )
            .await
            .unwrap();
